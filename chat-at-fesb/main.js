...
app.on('ready', ()=> {
    ...
    Notify.setConfig({
        appIcon: url.format({
            pathname: path.join(__dirname, 'windows/app/img/communication.svg'),
            protocol: 'file:',
            slashes: true
        })
    })
...
function createLoadKeysWindow(mainWindow) {
    ...
    loadKeysWindow.loadURL(url.format({
        pathname: path.join(__dirname, 'windows/loadkeys/index.html'),
        protocol: 'file:',
        slashes: true
    }))
